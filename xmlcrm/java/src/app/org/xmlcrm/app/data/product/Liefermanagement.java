...
//			Query query = session
//					.createQuery("select c from lieferarten as c where c.LIEFER_ID = :LIEFER_ID");
...
//			lieferarten.setStarttimeDE(CalenderI.getDatumMili(lieferarten
//					.getStarttime()));
...
//				Transaction tx = session.beginTransaction();
//				Query query = session
//						.createQuery("select c from lieferarten as c where c.freigeschalten = :freigeschalten");
...
//					lieferarten[vars].setStarttimeDE(CalenderI
//							.getDatumMili(lieferarten[vars].getStarttime()));
//					lieferarten[vars].setUpdatetimeDE(CalenderI
//							.getDatumMili(lieferarten[vars].getUpdatetime()));
//				}
			} catch (HibernateException ex) {
