...
public class Productmanagement {
	...
	public products[] getProductsByCat(){
	    ...
	    try {
			...
			Query query = session.createQuery("select c from products as c group by c.category");
	...
	public products[] searchProduct(String searchstring){
	    ...
	    try {
	        ...
	        for(Iterator it = contactsZ.iterator();it.hasNext();){
	        	...
	        	System.out.println("SearchArtnumm: "+products[k].getArtnumber());
	        	k++;
	        }
	    	HibernateUtil.closeSession();
	    } catch( HibernateException ex ) {
	...
	public products[] getProductsByCat(int start, int max, String cat){
	    ...
	    try {
	    	Session session = HibernateUtil.currentSession();
	    	Transaction tx = session.beginTransaction();
			Query query = session.createQuery("select c from products as c where c.category = :category");
	...
	public products getProductsByID(int artnumber){
	    ...
	    try {
	    	Session session = HibernateUtil.currentSession();
	    	Transaction tx = session.beginTransaction();
			Query query = session.createQuery("select c from products as c where c.artnumber = :artnumber");
