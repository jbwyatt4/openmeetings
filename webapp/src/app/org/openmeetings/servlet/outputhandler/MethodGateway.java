...
public class MethodGateway  extends HttpServlet {
	...
	private static final Log log = LogFactory.getLog(MethodGateway.class);
		...
		try {
			...
			if (service.equals("userservice")) {
				...
				if (method.equals("getSession")) {
					...
					String profilePictureUrl = httpServletRequest.getParameter("profilePictureUrl");
...
//						Integer filesPanelWidth) {
				...
				if (method.equals("addRoom")) {
					...
					Long roomtypes_id = Long.valueOf(httpServletRequest.getParameter("roomtypes_id")).longValue();
