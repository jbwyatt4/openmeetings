...
public class StreamService implements IPendingServiceCallback {
	 ...
	 * 
	 * the temp-value contains an attribute called *streamlist* the streamlist
	 * all room-streaming events are inside avset means this is an streaming
	 * event where only the audio-video settings have been altered
	 * 
	 * Meaning of the boolean Values in the *streamlist* : streamstart = true &&
	 * avset = false => an initial user (a user which has been already in the
	 * room when you started the recorder-session) streamstart = true && avset =
	 * true => new user did start streaming streamstart = false && avset = true
	 * => new user did has chosen his av/settings, if he has chosen *n*
	 * (rcl.avsettings == "n") then he will not send any streamstart event cause
	 * he does not stream audio/video => show the picture of the user
	 * streamstart = false && avset = false => user did finished streaming if
	 * the user has rcl.avsettings != "n" stop playing the stream
	 * 
	 * 
	 * Meaning of the boolean Values in the *roomclients* : roomenter = true =>
	 * a new user OR initial user roomenter = false => a user has left the
	 * Conference
	 * 
	 ...
	 * @return
	 */
	public String recordMeetingStream(String conferenceType,
			Object initwhiteboardvars, String roomRecordingsTableString,
			String comment) {
		try {
			...
			String recordingName = generateFileName(Long.valueOf(
					currentClient.getBroadCastID()).toString());
			...
			roomRecording.setRoom_setup(roommanagement
					.getRoomById(currentClient.getRoom_id()));
			roomRecording
					.setRoomRecordingsTableString(roomRecordingsTableString);
					...
					if (conn != null) {
						if (conn instanceof IServiceCapableConnection) {
							...
							if (room_id.equals(rcl.getRoom_id())
								...
								if (!conferenceType.equals("audience")
										|| rcl.getIsMod()) {

									RoomStream roomStream = new RoomStream();

									String streamName = generateFileName(Long
											.valueOf(rcl.getBroadCastID())
											.toString());
									...
									if (roomRecording.getRoomStreams() == null) {
										roomRecording
												.setRoomStreams(new LinkedList<RoomStream>());
									}
									roomRecording.getRoomStreams().add(
											roomStream);
								}

								// add room Clients enter/leave Events to record
								// File
								...
								if (roomRecording.getRoomClients() == null) {
									roomRecording
											.setRoomClients(new LinkedList<RecordingClient>());
								}
			...
			roomRecording.setInitwhiteboardvarsInXml(xStream_temp
					.toXML(initwhiteboardvars));
	...
	public Long _stopRecordMeetingStream(String roomrecordingName) {
		try {
			...
			return _stopRecordAndSave(current.getScope(), roomrecordingName,
					currentClient);
		} catch (Exception err) {
	...
	public Long _stopRecordAndSave(IScope scope, String roomrecordingName,
		...
		try {
					...
					if (conn != null) {
						if (conn instanceof IServiceCapableConnection) {
							...
							if (room_id.equals(rcl.getRoom_id())
								...
								if (!conferenceType.equals("audience")
										|| rcl.getIsMod()) {
									// stop the recorded flv and add the event
									// to the notifications
									log.debug("*** sendClientBroadcastNotifications Any Client is Recording - stop that");
									StreamService._addRoomClientEnterEventFunc(
											rcl, roomrecordingName,
											rcl.getUserip(), false);
									_stopRecordingShowForClient(conn, rcl,
											roomrecordingName, true);
								}
							}
			...
			String newRecordFileName = roomRecording
					.getRoomRecordingsTableString();
			...
			// //make persistent
			// Long recording_id =
			// RecordingDaoImpl.getInstance().addRecording(newRecordFileName,
			// duration, "", currentClient.getRoom_id(), us, comment, null);
			//
			// //save XML to Disk
			// IScope scope = Red5.getConnectionLocal().getScope().getParent();
			// String current_dir =
			// scope.getResource("upload/").getFile().getAbsolutePath();
			// //System.out.println(current_dir + File.separatorChar +
			// this.folderForRecordings);
			// File f = new File(current_dir + File.separatorChar +
			// folderForRecordings);
			// if (!f.exists()){
			// f.mkdir();
			// }
			// String fileName = f.getAbsolutePath() + File.separatorChar +
			...
			// pw.close();
			//
			// List<LinkedHashMap<String,Object>> roomClients =
			// (List<LinkedHashMap<String,Object>>)roomRecording.get("roomclients");
			// for (Iterator<LinkedHashMap<String,Object>> iter =
			// roomClients.iterator();iter.hasNext();){
			...
			// make persistent
			Long recording_id_remote = recordingDao.addRecording(
					newRecordFileName, duration, "",
					currentClient.getRoom_id(), us, comment,
					roomRecordingRemote);
	...
	/**
	 * if doStopStream = false this use is already away, no connection can be
	 * stoped
	 * 
	 * @param conn
	 * @param rcl
	...
	public void _stopRecordingShowForClient(IConnection conn,
		...
		try {
			...
			roomStream.setStarttime(currentDate.getTime()
					- recordingsStartTime.getTime());
			roomStream.setRcl(rcl);
	...
	public List<Recording> getAllRecordingsForUser(String SID) {
		try {
			...
			if (authLevelManagement.checkUserLevel(user_level)) {
				...
				int i = 0;
				List<Rooms> rooms = roommanagement.getPublicRooms(user_level);
				...
				for (Iterator<Recording> iter = rList.iterator(); iter
						.hasNext();) {
	...
	public void _addRecordingByStreamId(IConnection conn,
		...
		try {
			...
			if (!conferenceType.equals("audience") || rcl.getIsMod()) {
				...
				String streamName = generateFileName(Long.valueOf(
						rcl.getBroadCastID()).toString());
				String remoteAdress = conn.getRemoteAddress();
	...
	public static void addWhiteBoardEvent(String roomrecordingName, Object vars) {
		try {
			...
			log.debug("addWhiteBoardEvent roomrecordingName: "
					+ roomrecordingName);
			RoomRecording roomRecording = roomRecordingList
					.get(roomrecordingName);
			...
			whiteBoardEvent.setStarttime(currentDate.getTime()
					- recordingsStartTime.getTime());
	...
	public static void addChatEvent(String roomrecordingName, Object vars) {
		try {
			...
			chatvaluesEvent.setStarttime(currentDate.getTime()
					- recordingsStartTime.getTime());
	...
	public static void _addRoomClientAVSetEvent(RoomClient rcl,
			String roomrecordingName, String remoteAdress) {
		try {
			RoomRecording roomRecording = roomRecordingList
					.get(roomrecordingName);
			Date recordingsStartTime = roomRecording.getStarttime();
	...
	public static void _addRoomClientEnterEventFunc(RoomClient rcl,
			String roomrecordingName, String remoteAdress, boolean enter) {
		try {
			RoomRecording roomRecording = roomRecordingList
					.get(roomrecordingName);
			Date recordingsStartTime = roomRecording.getStarttime();
			...
			roomClient.setStarttime(currentDate.getTime()
					- recordingsStartTime.getTime());
			//
			// if (!enter) {
					...
					if (cons != null) {
						if (cons instanceof IServiceCapableConnection) {
							if (!cons.equals(current)) {
								...
								// same domain except its the current one
								if (room_id.equals(rcl.getRoom_id())
										&& room_id != null) {
	...
	public Long deleteRecordedFile(String SID, Long recording_id) {
		try {
			...
			if (authLevelManagement.checkUserLevel(user_level)) {
				...
				if (rec != null) {
					rec.setDeleted("true");
					recordingDao.updateRecording(rec);
