...
public class Contactmanagement {
	...
	public Contacts[] searchContactsByUser(int USER_ID,long USER_LEVEL,String searchstring,String searchcriteria,int searchMax, int start){
		Contacts contacts[] = new Contacts[1];
		if (USER_LEVEL>1){
			try {
				Session session = HibernateUtil.currentSession();
	...
	public Contacts[] searchContact(long USER_LEVEL,String searchstring,String searchcriteria,int searchMax, int start){
		Contacts contacts[] = new Contacts[1];
		if (USER_LEVEL>2){
			try {
		    	Session session = HibernateUtil.currentSession();
	...
	public Contacts searchContactByID(int CONTACT_ID,String searchstring,String searchcriteria,int searchMax, int start){
...
//                contacts.setConstyle(1);
	...
	public String updateContact(long USER_LEVEL,int USER_ID,int CONTACT_ID,String firstname, String lastname, String adress,String zip,String town,String state,String comment,String telefon, String fax,String mobil,String title,int EMailID, String email, int FREIGABE_ID){
		...
		if (USER_LEVEL>1){
			try {
	            ...
	            int updatedEntities = session.createQuery( hqlUpdate )
	                                ...
	                                .setLong( "age", Calender.getInstance().getTimeStampMili() )	 
	                                .setString("adress",adress)
	                                .setString("zip",zip)
	                                .setString("town",town)
	                                .setString("state",state)
	                                .setString("comment",comment)
	                                .setString("telefon",telefon)
	                                .setString("fax",fax)
	                                .setString("mobil",mobil)
	                                .setString("title",title)
	                                .setInteger("FREIGABE_ID",FREIGABE_ID)
	                                .setInteger( "CONTACT_ID", CONTACT_ID )	                                
	                                .executeUpdate();
	            res = "Success"+updatedEntities;
	            tx.commit();
	            HibernateUtil.closeSession();
	            res = Emailmanagement.getInstance().updateContactEmail(EMailID,CONTACT_ID,email);
	        } catch( HibernateException ex ) {
	        	log.error(ex);
	        } catch ( Exception ex2 ){
	        	log.error(ex2);
	...
	public String addContact(long USER_LEVEL,int USER_ID,String firstname, String lastname, String adress,String zip,String town,String state,String comment,String telefon, String fax,String mobil,String title, String email, int FREIGABE_ID){
...
//		    	}		    	
	...
	public congroups searchContactGroupsByID(long USER_LEVEL,int USER_ID,int CONGROUP_ID,String searchstring,String searchcriteria,int searchMax, int start){
...
//	    		}
	...
	private  Contactgroups[] searchContactsByGroup(int CONGROUP_ID,String searchstring,String searchcriteria,int searchMax, int start){
...
//			}
	...
	public congroups getContactGroupsByID(long USER_LEVEL,int CONGROUP_ID,int USER_ID,int val){
		congroups congroups = new congroups();
...
//		    	}
	...
	private congroups getGroupsByContact(int CONGROUP_ID){
		...
		try {
	    	Session session = HibernateUtil.currentSession();
	    	Transaction tx = session.beginTransaction();    
	    	Query query = session.createQuery("select c from congroups as c where c.CONGROUP_ID = :CONGROUP_ID");
			query.setInteger("CONGROUP_ID", CONGROUP_ID);
			for (Iterator it2 = query.iterate(); it2.hasNext();) {
				congroups = (congroups) it2.next();
			}
	...
	public String addContactGroup(long USER_LEVEL,long USER_ID,String name,int freigabe,String description,String comment){
...
//			congroups.setUSER_ID(USER_ID);
	...
	public String updateConGroup(long USER_LEVEL,int USER_ID,int CONGROUP_ID,String name,int freigabe,String description,String comment){
		...
		if (USER_LEVEL>1){
			try {
		        ...
		        int updatedEntities = session.createQuery( hqlUpdate )
		        					.setString( "name", name )
		        					.setInteger( "freigabe",freigabe )
		        					.setString( "description", description )
		                            .setLong( "updatetime", Calender.getInstance().getTimeStampMili() )
		                            .setString( "comment", comment )
		                            .setInteger( "CONGROUP_ID", CONGROUP_ID )
		                            .setInteger( "USER_ID", USER_ID )
		                            .executeUpdate();
		        //session.flush();
	...
	public String deleteContactgroup(int CONGROUP_ID,long USER_LEVEL, int USER_ID){
...
//	    	}
	...
	private boolean getCheckDuplicate(int CONTACT_ID,int CONUSER_ID,int CONGROUP_ID){
		boolean ret = true;
		try {
//	    	Session session = HibernateUtil.currentSession();
//	    	Transaction tx = session.beginTransaction(); 
...
//	    	}
	...
	public String addContactToGroup(long User_Level,int CONTACT_ID,int CONUSER_ID,int CONGROUP_ID, String comment){
		String result = "Fehler im Contactmanagement addContactToGroup";
		if (User_Level>1){

			 boolean NewFlag = getCheckDuplicate(CONTACT_ID, CONUSER_ID,CONGROUP_ID);
...
//            }
	...
	public Contactgroups getContactsByUID(int UID){
		...
		try {
	    	Session session = HibernateUtil.currentSession();
	    	Transaction tx = session.beginTransaction();    
	    	Query query = session.createQuery("select c from contactgroups as c where c.UID = :UID");
...
//			
	...
	public String updateContactGroup(long User_Level,int UID,int CONGROUP_ID, String comment){
		String result = "Fehler im Contactmanagement addContactToGroup";
		if (User_Level>1){
	...
	public String deleteContactFromGroup(int USER_ID,long USER_LEVEL,int CONTACT_ID,int CONUSER_ID,int CONGROUP_ID){
            ...
            if (USER_ID == congroups.getUSER_ID()){
    			try {
    		        ...
    		        int deletedEntities = session.createQuery( hqlDelete )
    		                            .setInteger( "CONGROUP_ID", CONGROUP_ID )
    		                            .setInteger( "CONTACT_ID", CONTACT_ID )
    		                            .setInteger ( "CONUSER_ID", CONUSER_ID )
    		                            .executeUpdate();
    	            tx.commit();
    	            HibernateUtil.closeSession();
    	            result = "Der Contact wurde erfolgreich von der Group gelöscht"+deletedEntities;
    			} catch( HibernateException ex ) {
    	        	result += "Error: "+ex;
    	        } catch ( Exception ex2 ){
    	        	result += "Error: "+ex2;
                ...
                System.out.println("**** Security Log USER TRIES TO MOVE OHTERS - CONGROUP_ID: "+CONGROUP_ID+" | USER_ID:  "+USER_ID+" | CONTACT_ID: "+CONTACT_ID);
            }
		}else {
			result = "Permission denied";
	    }
	...
	public String deleteContactFromAllGroups(int CONTACT_ID){
		...
		try {
            ...
            tx.commit();
            HibernateUtil.closeSession();
            result = "Der Contact wurde erfolgreich von der Group gelöscht"+deletedEntities+" CONTACT_ID:";
		} catch( HibernateException ex ) {
        	result += "Error: "+ex;
        } catch ( Exception ex2 ){
        	result += "Error: "+ex2;
	...
	public String deleteGroupsFromAllContacts(int CONGROUP_ID){
		...
		try {
	        ...
	        int deletedEntities = session.createQuery( hqlDelete )
	                            .setInteger( "CONTACT_ID", CONGROUP_ID )
	                            .executeUpdate();
            tx.commit();
            HibernateUtil.closeSession();
            result = "Der Contact wurde erfolgreich von der Group gelöscht"+deletedEntities;
