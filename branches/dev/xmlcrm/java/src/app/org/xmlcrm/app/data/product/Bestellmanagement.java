...
//		}
	...
	public String updateWarenkorb (int WAREN_ID, int status, int ZAHLUNGS_ID, int LIEFER_ID, int amount, String  comment){
		String result = "Fehler im Bestellvorgang";
	    try {
	    	Session session = HibernateUtil.currentSession();
	    	Transaction tx = session.beginTransaction();    	
	        ...
	        int updatedEntities = session.createQuery( hqlUpdate )
	                            .setLong( "updatetime", CalenderI.getTimeStampMili() )
	                            .setInteger( "status", status )
	                            .setInteger( "ZAHLUNGS_ID", ZAHLUNGS_ID )
	                            .setInteger( "LIEFER_ID", LIEFER_ID )
	                            .setString( "comment", comment )
	                            .setInteger( "menge", amount )
	                            .setInteger( "WAREN_ID", WAREN_ID )
	                            .executeUpdate();
	        //session.flush();
...
//		    }
	...
	public int countUserStatus(int USER_ID,int status){
		...
		try {
	    	Session session = HibernateUtil.currentSession();
	    	Transaction tx = session.beginTransaction();    
	    	Query query = session.createQuery("select c from userwaren as c where c.USER_ID = :USER_ID AND c.status = :status");
			query.setInteger("USER_ID", USER_ID);
	...
	private transstatus getTransstatus(int STATUS_ID){
		...
		try {
	    	Session session = HibernateUtil.currentSession();
	    	Transaction tx = session.beginTransaction();    
	    	Query query = session.createQuery("select c from transstatus as c where c.STATUS_ID = :STATUS_ID");
